    ...
    while (cont)
    {
        ...
        for (int i = 0; i < num_rows_entrenamiento; i++)
        {
            ...
            for (int j = 0; j < num_salidas; j++)
            {
                ...
                float new_sesgo = old_sesgo + (tasa_aprendizaje * (t - y));
                entrada_raw[0].conexiones[j].peso = new_sesgo;

                if(abs(old_sesgo-new_sesgo) > tol)
                    cont = true;
                ...
                for (int k = 1; k < num_entradas; k++)
                {
                    ...
                    float new_peso = old_peso + (tasa_aprendizaje * (t - y) * entrada[k - 1]);
                    entrada_raw[k].conexiones[j].peso = new_peso;

                    if(abs(old_peso-new_peso) > tol)
                        cont = true;
